...
import UploadedImageExport from 'src/app/models/uploaded-image-export';
import User from 'src/app/models/user';
import databaseConnection from './database';
import TextRegionDao from './region-dao';
import UserDao from './user-dao';
import { toOrdinal } from 'pg-parameterize';
import { ImageTag } from 'src/app/models/image-tag';
import ImageTypeDao from './image-type-dao';
...
class ImageDao {
  ...
  private readonly imageTypeDao = ImageTypeDao.getInstance();
  //   private readonly imageTagDao = ImageTagDao.getInstance();
  ...
  public async getImagesExport(
    ...
    try {
      ...
      const imageId2Tags = new Map<string, ImageTag[]>(
        imageResultIds.map((item) => [item, []])
      );
      const imageResultsTags = await databaseConnection.any(
        toOrdinal(`
            SELECT * FROM public."ImageHasTag"
            JOIN public."ImageTags" ON "ImageHasTag"."imageTagId" = "ImageTags"."imageTagId"
            WHERE "ImageHasTag"."imageId" IN (${imageResultIds
              .map(() => '?')
              .join(',')});
        `),
        imageResultIds
      );
      imageResultsTags.forEach((item) => {
        imageId2Tags.get(item.imageId).push(ImageTag.parseFromJson(item));
      });

      const imageTypes = await this.imageTypeDao.getAllImageTypesWithoutLabel();
      const id2ImageTypes = new Map(
        imageTypes.map((item) => [item.typeId, item])
      );
      const images: UploadedImageExport[] = [];
      for (const item of imageResults) {
        const ofUser = User.parseFromJson(item);
        const imageType = item.imageType
          ? id2ImageTypes.get(item.imageType)
          : null;
        images.push(
          new UploadedImageExport(
            item.imageId,
            imageType,
            item.imageUrl,
            item.thumbnailUrl,
