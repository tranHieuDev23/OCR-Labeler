...
import { RegionLabel } from 'src/app/models/image-type';
...
class TextRegionDao {
  ...
  public async getRegionLabelOfType(typeId: string) {
    try {
      const labelResults = await databaseConnection.manyOrNone(
        `
                SELECT * FROM public."RegionLabels"
                    WHERE "RegionLabels"."ofTypeId" = $1;
            `,
        [typeId]
      );
      if (!labelResults) {
      ...
      return labelResults.map((item) => RegionLabel.parseFromJson(item));
    } catch (e) {
      throw new Error(
        `[getRegionLabelOfType()] Error happened while reading region labels from database: ${e}`
      );
    }
  }

  public async addRegionLabels(label: RegionLabel[], ofTypeId: string) {
    try {
      if (!label || label.length === 0) {
        return;
      }
      const cs = new pgp.helpers.ColumnSet(
        ['labelId', 'ofTypeId', 'displayName', 'color'],
        { table: 'RegionLabels' }
      );
      const query = pgp.helpers.insert(
        label.map((item) => {
      ...
      await databaseConnection.none(query);
    } catch (e) {
      throw new Error(
        `[addRegionLabels()] Error happened while reading region label from database: ${e}`
      );
    }
  }
}
